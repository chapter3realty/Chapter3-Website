...
function main() {
  ...
  /* EIA-861 average monthly RESIDENTIAL ELECTRICITY bill, by state. Real
   * observed bills, not an index scaled off a national average. A state
   * average, so a metro spanning two states gets its primary state's figure
   * and the page says so. See data/relocating/eia-bills.json for the source. */
  const EIA = require('./eia-bills.json');
  const elecOf = (abbr) => {
    const r = EIA.states[abbr || 'US'];
    return r ? r.monthlyBill : null;
  };
...
function buildAsset(d) {
  const rows = d.places.map(p => [p.id, p.name, p.state, p.kind, p.rpp.all, p.rpp.goods, p.rpp.housing, p.rpp.utilities, p.rpp.other, p.zhvi, p.zori, p.coli, p.elec == null ? null : p.elec]);
  const m = { rppVintage: d.meta.rppVintage, zillowLatestMonth: d.meta.zillowLatestMonth, elecVintage: d.meta.elecVintage, generated: d.meta.generated, counts: d.meta.counts };
  return '/* Cost of living data for /buyers/relocating/cost-of-living/. Generated by data/relocating/build-col-data.js. Do not edit by hand.\n' +
    '   Sources: ' + d.meta.rppSource + '; ' + d.meta.zillowSource + '; ' + (d.meta.elecSource || '') + '.\n' +
    '   Row: [id, name, state, kind, rppAll, rppGoods, rppHousing, rppUtilities, rppOther, zhvi, zori, coli, elecBill]; US = 100.\n' +
    '   elecBill: average monthly residential electricity bill for the state, US dollars. */\n' +
