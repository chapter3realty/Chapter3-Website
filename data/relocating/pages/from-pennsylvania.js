...
<section style="background:var(--ivory)"><div class="wrap"><p style="${S.eyebrow}">The rest of the bill</p><h2 style="${S.h2}">The car, the clothes and the sales tax</h2><p style="${S.pLast}">Two smaller lines move against you. South Carolina taxes every vehicle on its value each year, roughly $360 to $460 on a $30,000 car plus a $50 road fee, where Pennsylvania charges a flat registration; count on a one-time $250 fee per vehicle on arrival and a 45 day window to register. And clothing, which Pennsylvania never taxes, carries the full 8 to 9 percent sales tax here. Deadlines and office addresses are on the ${A('/buyers/relocating/moving-checklist/', 'new resident checklist')}.</p></div></section>
...
<h2 style="font-family:var(--serif);font-size:1.9rem;font-weight:300;color:var(--ivory);letter-spacing:-.01em;margin-bottom:.6rem;text-align:center">Tell us what you pay in Pennsylvania.</h2>
<p style="font-family:var(--sans);font-size:.93rem;color:rgba(244,239,232,.68);margin:0 auto 1.9rem;line-height:1.65;text-align:center;max-width:52ch">Send your county and price range. We will put your property tax and wage tax next to the real bills on a house here, in both directions, with nothing rounded to flatter the move.</p>
<div id="ldWrap">
<div style="display:grid;gap:.7rem;margin-bottom:1rem">
<input class="ld-in" id="ldCtx" placeholder="Your Pennsylvania county, and your price range here">
...
<button class="btn btn-brass" style="width:100%;justify-content:center" onclick="ldSubmit()">Send my PA numbers</button>
...
function ldSubmit(){
 ...
 c3SendForm({property_address:cx,name:n,phone:ph,email:em,consent:c?'yes':'no'},'relocating-from-pennsylvania');
