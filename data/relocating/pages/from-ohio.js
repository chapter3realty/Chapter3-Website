...
const faq = [
  { q: 'Is Myrtle Beach cheaper than Ohio?',
    a: 'Mostly, not entirely. Local income tax disappears, gas runs about 10 cents cheaper, and property tax rates are far lower. Against that, a typical house costs more than the Cleveland area, sales tax is higher, and the yearly car tax is new. The calculator on our cost of living page runs your exact city.' },
  ...
  { q: 'Does South Carolina really tax my car every year?',
    a: 'Yes. Vehicles are taxed yearly on value: a $30,000 car runs roughly $360 to $460 plus a $50 road fee, and the bill must be paid before the plate renews. Ohio charges a flat fee of roughly $36 to $74. Add a one-time $250 fee per vehicle when you arrive, within 45 days.' },
  ...
  { q: 'What surprised Devin most after moving from Ohio?',
    a: 'The county vehicle bill, first: $325 on his car, plus the one-time $250 fee and the $50 road fee, none of which Ohio charges. Then two he did not see coming: needing sunglasses daily in summer, and a job market far wider than Pleasant City, the village of about 400 he left.' },
];
...
<section style="background:var(--ivory-2)"><div class="wrap"><p style="${S.eyebrow}">Income tax</p><h2 style="${S.h2}">The tax that disappears is the local one</h2><p style="${S.p}">Ohio&#39;s state rate is one of the lowest in the country. What most Ohio households actually feel is the layer under it: city income taxes, commonly 1 to 3 percent of wages, and in some places a school district income tax on top of that.</p><p style="${S.pLast}">South Carolina has no local income tax anywhere. No city rate, no district rate. For a working household this is the largest single income-tax change in the whole comparison, and it is bigger than the difference between the two state rates.</p></div></section>

<section style="background:var(--ivory)"><div class="wrap"><p style="${S.eyebrow}">Retirees</p><h2 style="${S.h2}">Retirement income: the clearest win of any state we compare</h2><p style="${S.p}">Ohio taxes pensions, 401(k) and IRA withdrawals at its flat rate. The relief is a retirement credit that tops out at $200 a year, one per return, and disappears entirely at $100,000 of income, plus a $50 credit at 65.</p><p style="${S.pLast}">South Carolina works from deductions instead: up to $3,000 of retirement income before 65 and up to $10,000 from 65, per person, plus a separate age-65 deduction of up to $15,000 against any income. Social Security is untaxed in both states. For most retirees the same pension is simply taxed less here, and the property tax on the house drops at the same time.</p><div style="${S.ctaBox}"><p style="margin:0;color:var(--navy);font-family:var(--sans);font-size:.92rem;font-weight:600">Retiring here from Ohio? We will run your whole picture.</p><div style="display:flex;gap:.7rem;flex-wrap:wrap"><a class="btn btn-brass" href="#lead-form">Run my retirement math</a><a class="btn btn-outline" href="tel:+18543332135">Call 854.333.2135</a></div></div></div></section>

<section style="background:var(--ivory-2)"><div class="wrap"><p style="${S.eyebrow}">The other direction</p><h2 style="${S.h2}">Two things cost more here, and Devin hit both</h2><h3 style="${S.h3}">South Carolina taxes your car every year</h3><p style="${S.p}">Ohio charges a flat registration of roughly $36 plus small local fees, and nothing based on the car&#39;s value. Here every vehicle is taxed yearly on its value: a $30,000 car runs roughly $360 outside the city limits and closer to $460 inside Myrtle Beach, plus a $50 road fee, and the county bill must be paid before the plate renews. Arriving adds a one-time $250 fee per vehicle, with 45 days to register. A two-car household should plan on roughly $500 to $1,000 a year that did not exist in Ohio. The bill for boats and campers works the same way. Devin, who wrote this page, arrived from Ohio three years ago, and this tax was the item that caught him. The steps, in order, are on the ${A('/buyers/relocating/moving-checklist/', 'new resident checklist')}.</p><h3 style="${S.h3}">Houses and sales tax</h3><p style="${S.pLast}">A typical home here costs about $342,000 against about $254,000 in the Cleveland area, so the purchase itself is a step up for many Ohio buyers even though the tax rate on it is far lower. Sales tax runs 8 percent in the county and 9 percent in the city, against about 7.3 percent combined in Ohio. Groceries are untaxed in both.</p></div></section>
...
<h2 style="font-family:var(--serif);font-size:1.9rem;font-weight:300;color:var(--ivory);letter-spacing:-.01em;margin-bottom:.6rem;text-align:center">Moving from Ohio? Ask the one who did.</h2>
<p style="font-family:var(--sans);font-size:.93rem;color:rgba(244,239,232,.68);margin:0 auto 1.9rem;line-height:1.65;text-align:center;max-width:52ch">Send your Ohio city and your price range. You get the real numbers on a house here, plus the list of things Devin wishes he had known before his own move.</p>
<div id="ldWrap">
<div style="display:grid;gap:.7rem;margin-bottom:1rem">
<input class="ld-in" id="ldCtx" placeholder="Your Ohio city, and your target price range here">
...
<button class="btn btn-brass" style="width:100%;justify-content:center" onclick="ldSubmit()">Send my Ohio numbers</button>
...
function ldSubmit(){
 ...
 c3SendForm({property_address:cx,name:n,phone:ph,email:em,consent:c?'yes':'no'},'relocating-from-ohio');
 document.getElementById('ldWrap').style.display='none';
 document.getElementById('ldOk').style.display='block';
}
</script></div></section>

<section style="background:var(--ivory)"><div class="wrap"><p style="${S.eyebrow}">First hand</p><h2 style="${S.h2}">What surprised Devin after the move</h2><p style="${S.p}">Devin moved here from Pleasant City, a village of about 400 people in Guernsey County, Ohio. Three years in, three things stand out to him. The county vehicle bill, which Ohio never sends: his first one was $325, on top of the one-time $250 fee and the $50 road fee. Needing sunglasses: he never owned a pair that mattered in Ohio and reaches for them daily here in summer. And the work: coming from a village of 400, the range of jobs here read like a big city&#39;s.</p><p style="${S.pLast}">His own bills are quoted, labelled as one household&#39;s, on the ${A('/buyers/relocating/cost-of-living/', 'cost of living page')}, and his first-45-days mistakes are the reason the ${A('/buyers/relocating/moving-checklist/', 'checklist')} exists.</p></div></section>
