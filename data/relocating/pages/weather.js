...
const spec = {
  ...
  description: 'Myrtle Beach weather from NOAA normals: season by season temperatures, ocean water by month, real humidity and sunshine figures, and how often it snows.',
...
<div style="overflow-x:auto;-webkit-overflow-scrolling:touch"><table style="width:100%;border-collapse:collapse;font-family:var(--sans);font-size:.9rem;color:var(--navy);min-width:520px"><thead><tr><th style="${S.th}">Months</th><th style="${S.th}">Water temperature</th><th style="${S.th};border-bottom:0" aria-hidden="true"></th></tr></thead><tbody>
<tr><td style="${S.td}">January and February</td><td style="${S.td}">around 50</td><td style="border:0;padding:0"></td></tr>
<tr><td style="${S.td}">March and April</td><td style="${S.td}">upper 50s to upper 60s</td><td style="border:0;padding:0"></td></tr>
<tr><td style="${S.td}">May</td><td style="${S.td}">mid 70s</td><td rowspan="3" style="border:0;border-left:3px solid var(--brass);padding:.2rem .1rem .2rem .55rem;vertical-align:middle"><span style="display:inline-block;writing-mode:vertical-rl;transform:rotate(180deg);font-family:var(--sans);font-size:.64rem;letter-spacing:.14em;text-transform:uppercase;color:var(--brass-ink);white-space:nowrap">Swim season</span></td></tr>
...
<tr><td style="${S.td}">November</td><td style="${S.td}">mid 60s</td><td style="border:0;padding:0"></td></tr>
</tbody></table></div>
<p style="${S.small};margin-top:.9rem">NOAA Springmaid Pier gauge, station 8661070, hourly readings averaged by us across 2024 and 2025. The bracket marks May into October, when the water is at or above 70 degrees, the low end of what the federal coastal water temperature guide calls comfortable for most swimmers.</p></div></section>

<section style="background:var(--ivory)"><div class="wrap"><p style="${S.eyebrow}">Humidity and sun</p><h2 style="${S.h2}">The two numbers people argue about, sourced</h2><h3 style="${S.h3}">The humidity, in numbers</h3><p style="${S.p}">Myrtle Beach itself has no long term government humidity record, so the honest numbers come from the nearest stations that do. Wilmington&#39;s long term record shows morning humidity around 85 to 88 percent in July and August, and the closest hourly station puts July dew points around 73 degrees. That combination is why an 87 degree afternoon here asks more of you than the same number inland. Summers are air conditioned, and the ${A('/buyers/relocating/cost-of-living/', 'cost of living calculator')} carries the real electricity bills that go with that.</p><h3 style="${S.h3}">The sunny days number you see everywhere is not a government number</h3><p style="${S.pLast}">Websites repeat that Myrtle Beach gets about 215 sunny days a year. That figure comes from commercial estimates, not from NOAA, and no government station publishes it. What the nearest long term government station does show: 62 percent of possible sunshine and about 111 clear days a year. On that same government measure, Buffalo records 54 clear days, Pittsburgh 59 and Cleveland 66. The honest version is still a large difference; it just is not 215.</p></div></section>

<section style="background:var(--ivory-2)"><div class="wrap"><p style="${S.eyebrow}">Snow</p><h2 style="${S.h2}">It snows about an inch a year, and twice lately it mattered</h2><p style="${S.p}">The weather service keeps a snowfall database for Myrtle Beach going back to 1940. The total across those 85 years is 87.4 inches, an average of about one inch a year, and most years record none at all. The record is 14 inches, December 1989.</p><p style="${S.p}">Recent years have been livelier than the average suggests. Five inches fell in January 2025, which the weather service called the city&#39;s largest snowstorm in 35 years, and another five fell at the turn of February 2026. Plows are not a municipal habit here, so a snow day mostly means the town closes and children see the beach white, briefly.</p><p style="${S.pLast}">Devin, who wrote this page, moved here from Ohio expecting to leave snow behind. It snowed his first winter, and he jokes that he brought it with him. His real measure of winter is his motorcycle: he rides into late November or early December, what finally stops him is the cold night ride home rather than the daytime, and the bike stays parked for about three months. He is back on it around the middle of March. Winters here are mild next to Ohio, and still not shorts weather unless the sun is out and you have some tolerance.</p><div style="${S.ctaBox}"><p style="margin:0;color:var(--navy);font-family:var(--sans);font-size:.92rem;font-weight:600">Want to talk about timing a move around the seasons?</p><div style="display:flex;gap:.7rem;flex-wrap:wrap"><a class="btn btn-brass" href="#lead-form">Talk through my move</a><a class="btn btn-outline" href="tel:+18543332135">Call 854.333.2135</a></div></div></div></section>
