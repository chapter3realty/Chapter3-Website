...
<section style="background:var(--ivory)"><div class="wrap"><p style="${S.eyebrow}">The short answer</p><h2 style="${S.h2}">What actually differs between them</h2><p style="${S.p}">People search Myrtle Beach and then buy somewhere else on this list, because the name covers about sixty miles of coast plus the inland towns behind it. The differences that change a purchase are these four, in this order.</p><p style="${S.p}"><strong style="color:var(--navy)">Whether the address is inside a city or town, or in unincorporated county.</strong> That one line decides your sales tax, your trash collection, your beach parking decal, and who writes the zoning, golf cart and rental rules that apply to you.</p><p style="${S.p}"><strong style="color:var(--navy)">What your budget reaches.</strong> The starting price in Conway and the starting price on Pawleys Island are more than $300,000 apart.</p><p style="${S.p}"><strong style="color:var(--navy)">Whether you can rent it out.</strong> This varies more between towns here than anything else, and an HOA can forbid what the zoning allows.</p><p style="${S.pLast}"><strong style="color:var(--navy)">The drive.</strong> To work, to a hospital, to the airport. Everything is closer than it looks on a map, until July.</p></div></section>
...
<h2 style="font-family:var(--serif);font-size:1.9rem;font-weight:300;color:var(--ivory);letter-spacing:-.01em;margin-bottom:.6rem;text-align:center">Tell us what the house has to do.</h2>
<p style="font-family:var(--sans);font-size:.93rem;color:rgba(244,239,232,.68);margin:0 auto 1.9rem;line-height:1.65;text-align:center;max-width:52ch">Give us your budget and what the house has to be near. We will come back with the towns that fit and the ones that do not, and the records behind both.</p>
<div id="ldWrap">
<div style="display:grid;gap:.7rem;margin-bottom:1rem">
<input class="ld-in" id="ldCtx" placeholder="Your budget, and what the house has to be near">
...
<button class="btn btn-brass" style="width:100%;justify-content:center" onclick="ldSubmit()">Shortlist my towns</button>
...
function ldSubmit(){
 ...
 c3SendForm({property_address:cx,name:n,phone:ph,email:em,consent:c?'yes':'no'},'relocating-which-town');
