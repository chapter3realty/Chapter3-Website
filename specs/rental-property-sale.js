 ...
 * research/invest-next/owner-answers-batch3.md (answers 1 to 6, round 2).
...
module.exports = {
  ...
  sections: [
    ...
    { h2: "Who can buy a house with a tenant in it?", html:
      h.p(`The lease sets the buyer pool. Most lenders require the buyer to move in ${h.ext(FHA, "within 60 days of closing")} on an FHA, VA or USDA loan. Devin Day, Chapter3's operations officer, sets the limit at three to four months left on the lease. Under it, an owner-occupant buyer can close and wait. Over it, the buyer is an investor, or you wait for the lease to end, or you pay the tenant to leave.`) +
      h.p("Condition then sets which investor buys. A house the tenant kept up sells to a turnkey buyer, who wants a house that rents the day after closing. A house the tenant damaged sells to a value-add buyer, who wants instant equity and will fix it before renting. The value-add buyer pays less, so the damage costs you the repair and the discount.") +
      h.p(`The third test is the price. The house cash-flowed at what you paid and at the taxes you were assessed. The buyer pays today's price, and the sale resets the assessed value at the 6 percent rate. A house that no longer cash flows at today's price still sells: Chapter3 has investors who buy a house in an appreciating area with modest cash flow. ${h.a("/buyers/property-taxes/", "How the sale resets the tax value")}.`) +
    ...
    { h2: "How do showings work with a tenant in place?", html:
      ...
      h.p(`Selling is not a ground to end a lease. ${h.ext(SC2737, "The three grounds for ejectment")} are unpaid rent, a term that has ended, and a lease violation. An eviction filed on any other basis while you market the house exposes you to a lawsuit, and the sale waits on it. A month-to-month tenancy is the exception: 30 days written notice ends it, for any reason.`) +
    ...
    { h2: "How do you price and market a rental?", html:
      h.p("Two buyer pools want different things. An investor wants the rent roll, the lease and the deposit ledger, and will buy with the tenant in place. An owner-occupant wants possession, so the lease end date sets when that pool can buy. Price to the pool the lease allows.") +
