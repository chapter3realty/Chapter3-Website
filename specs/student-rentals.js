 ...
 * attributed to Chapter3's licensed loan originator, without a name. The overlay example is illustrative and says so.
...
module.exports = {
  ...
  sections: [
    { h2: "What is a student rental?", html:
      ...
      h.p("Two tenant pools rent near campus, in Chapter3's files. Students want the cheapest house that fits their group. Families who move to Conway to be near a student pay a premium for a nice house close to campus. The second pool is smaller and steadier, and it rents the nicer houses.") },
    ...
    { h2: "How many students can live in one house?", html: (bg) =>
      h.p(`The rent depends on this number. Inside the City of Conway, ${h.ext(UDO, "the zoning code")} defines a family as related people or a group of not more than three unrelated persons living as one household. Three unrelated tenants is the cap in an ordinary residential zone, however many bedrooms the house has.`) +
    ...
    { h2: "What does a student house rent for?", html:
      h.p(`There is no public rent index for student houses in Conway. The one public benchmark by bedroom count is the federal fair market rent for Horry County. ${h.ext(FMR, "The federal housing agency publishes it")} at the 40th percentile of gross rents for standard-quality units. ${h.ext(FMR27, "For the federal fiscal year 2027")} it is $1,155 for a studio, $1,258 for one bedroom, $1,504 for two, $1,823 for three and $1,981 for four. Those are county-wide figures for ordinary rentals with utilities included, not student rents by the room.`) +
    ...
    { h2: "What do parents buy?", html:
      ...
      h.p(`Three things to settle first. The student cannot live there before junior year unless the university grants an exemption. The house is not the parent's legal residence, so it is taxed at the 6 percent rate. ${h.a("/buyers/property-taxes/", "How Horry County property tax is calculated")}. The lender decides how the loan is classified. Ask before you count on any program. Reviewed by Chapter3's licensed mortgage loan originator, NMLS 2721275.`) },
