 ...
 * research/invest-next/owner-answers-batch3.md (answers 7 to 13), attributed
 * to Devin Day. The overlay example is illustrative and says so. Built by
 * tools/mkpage.js. */
...
module.exports = {
  ...
  author: "tim",
  shortAnswer: "A student rental is a house or townhome rented to Coastal Carolina University students by the academic year. The university requires its first- and second-year students to live on campus, so the tenants are juniors, seniors and graduate students. Leases follow the calendar: classes begin in mid-August and end in early May. The City of Conway counts a household as no more than three unrelated people. The neighborhoods next to campus cap tenants at the bedroom count, up to four. About three in four student leases carry a parent co-signer. Parents often buy the house, rent the other rooms, and sell at graduation. The numbers work when the bedroom count, the cap and the rent per room line up. We run them before you offer.",
  sections: [
    ...
    { h2: "How do you screen a student tenant?", html:
      ...
      h.p("Be honest about the tenant. These are college students, and the house will be used the way you used one. If parties are a risk you can price, a house near campus is a good place to invest. Devin Day's tools for that risk are the short-lease premium, the parent co-signer and a larger deposit. The other two are an owner who lives nearby, or a multi-unit with a manager living in one unit. State law sets no maximum deposit.") },
    ...
    { h2: "Which houses and neighborhoods fit?", html: (bg) =>
      ...
      h.p("Plenty of communities here ban renting outright. We filter those out before you see a house. We then read the covenants and the association's rules for anything that breaks the strategy: a leasing cap, a minimum lease term, an occupancy limit, a parking rule. Devin Day calls that review the biggest value Chapter3 adds for an investor here.") +
