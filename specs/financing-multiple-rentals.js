...
module.exports = {
  ...
  author: "devin",
  shortAnswer: "Standard investment loans allow up to ten financed properties per borrower, counting your own home if it carries a mortgage. Each rental adds reserves: months of payments on the new one, and a share of the balances on the others. The first rental is the hardest to finance, because the loan rules count rent toward your income only after a year of managing property. Investor lenders often want experience too. After the first, the down payment on the next house can come from equity in one you own. The numbers must still work with both loans on them. We run the next house before you offer, and a loan officer at our preferred lender maps the loan.",
  sections: [
    ...
    { h2: "How many rentals can you finance?", html:
      h.p(`${h.ext(LIMIT, "Ten financed properties")} on standard loans, counting every one- to four-unit property with a mortgage in your name, including your own home. Commercial property, buildings with more than four units, and vacant lots do not count. A property with two mortgages counts once.`) +
    ...
    { h2: "What reserves does each rental add?", html:
      h.p(`${h.ext(RESERVES, "The rule asks for six months of payments")} in reserve on a new investment property. It also asks for a share of the balances on your other financed properties. The share is two percent with one to four financed properties, four percent with five or six, and six percent with seven to ten.`) +
    ...
    { h2: "Can the next purchase be funded from equity?", html: (bg) =>
      ...
      h.p(`It is still a down payment. The money is borrowed against one house to buy another, so two loans now depend on the rents. The refinanced house needs to carry its new payment from its own rent, and the new house needs to carry its own. If both do, you bought the second house without new cash from savings.`) +
    ...
    { h2: "What goes wrong on the second purchase?", html: (bg) =>
      h.p(`Expecting the same paperwork. Each lender and each underwriter asks for its own list, and a seller can ask for more too. Expect a different list from last time, even with the same broker. We ask for the least the file needs, and the underwriter still writes the list.`) +
      h.p(`Skipping the worst case. A second rental doubles the income and doubles the risk. Before you buy, write down what happens if both houses sit empty for three months, or if one needs a roof, and where that money comes from. We help clients build those defenses before they buy, so the worst case is unlikely, and survivable if it comes.`) +
      h.p(`In Chapter3's files a client planned to fix a house he owned, rent it, sell a few others and buy more. Fixing the house would have used every dollar he had. We showed him the risk. He sold a few houses first, then fixed the house, and bought fewer than he planned. He kept a reserve, and the plan held.`) +
  ...
  faq: [
    ...
    { q: "What reserves do I need for a second rental?", a: "Six months of payments on the new property, plus a share of the balances on your other financed properties. The share rises with the count under the standard loan rules. A loan officer at our preferred lender confirms the number for your file." },
  ],
  sources: [
